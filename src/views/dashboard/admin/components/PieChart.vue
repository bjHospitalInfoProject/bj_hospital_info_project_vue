...
import { getProvinceCount } from '@/api/dashboard'
...
export default {
  ...
  methods: {
    async getList() {
      var { data } = await getProvinceCount({ templateId: this.templateId })
      console.log(data)
      data = data.map(item => {
        return {
          name: item.name.replace("省", "").replace("自治区", ""),
    ...
    initChart(data) {
      ...
      //   { name: '山西', value: 150, color: '#00ff00' },   // 绿色
      // ];
      // 初始数据
      // const data = [
      //   { name: '北京', value: 123 },
