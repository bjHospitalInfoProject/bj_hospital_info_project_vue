<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      <el-table-column align="center" label="组织石白片ID">
        <template slot-scope="{row}">
          <span>{{ row.tissueSlideId }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="目前白片张数">
        <template slot-scope="{row}">
          <span>{{ row.slideCount }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="白片编号">
        <template slot-scope="{row}">
          <span>{{ row.slideNumber }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="出借情况">
        <template slot-scope="{row}">
          <span>{{ row.lendingStatus }}</span>
        </template>
      </el-table-column>
...
<script>
import { getTissueSlidePageInfoInfo } from '@/api/dataquery'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const { data } = await getTissueSlidePageInfoInfo(this.listQuery)
