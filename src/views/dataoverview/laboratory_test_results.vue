  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="就诊方式">
        <template slot-scope="{row}">
          <span>{{ row.treatmentModeStr }}</span>
        </template>
      </el-table-column>
...
<script>
import { getLabTestResultPageInfo } from '@/api/dataquery'
...
export default {
  ...
  methods: {
    getDetailInfoOption(row) {
      this.drawer = true;
      this.labortyInfo = { ...row }
    },
    async getList() {
      this.listLoading = true
      this.listQuery.centerId = this.centerId
      const { data } = await getLabTestResultPageInfo(this.listQuery)
