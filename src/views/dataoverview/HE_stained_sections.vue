<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      <el-table-column align="center" label="HE染色ID">
        <template slot-scope="{row}">
          <span>{{ row.heStainingSliceId }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="目前有无HE染色切片">
        <template slot-scope="{row}">
          <span>{{ getLabelFromValue(Options, row.hasSlice) }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="HE染色切片编号">
        <template slot-scope="{row}">
          <span>{{ row.sliceNumber }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="HE染色切片扫描图像">
        <template slot-scope="{row}">
          <span>{{ getLabelFromValue(Options, row.hasImage) }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="HE染色时间">
        <template slot-scope="{row}">
          <span>{{ row.stainingTime }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="HE染色结果描述">
        <template slot-scope="{row}">
          <span>{{ row.resultDescription }}</span>
        </template>
      </el-table-column>
...
<script>
import { getHEInfoList } from '@/api/dataquery'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const { data } = await getHEInfoList(this.listQuery)
