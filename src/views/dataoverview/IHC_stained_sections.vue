    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      <el-table-column align="center" label="IHC染色ID">
        <template slot-scope="{row}">
          <span>{{ row.ihcStainingSliceId }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="IHC染色切片编号1">
        <template slot-scope="{row}">
          <span>{{ row.sliceNumber1 }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="IHC染色切片编号2">
        <template slot-scope="{row}">
          <span>{{ row.sliceNumber2 }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="IHC染色目的蛋白">
        <template slot-scope="{row}">
          <span>{{ row.targetProtein }}</span>
        </template>
      </el-table-column>
...
<script>
import { getIHCInfoList } from '@/api/dataquery'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const { data } = await getIHCInfoList(this.listQuery)
